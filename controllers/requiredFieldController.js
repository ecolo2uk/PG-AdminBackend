...
export const getAllRequiredFields = async (req, res) => {
  try {
    const fields = await RequiredField.find({ status: "Active" });
...
export const deleteRequiredField = async (req, res) => {
  try {
    const deletedField = await RequiredField.findById(req.params.id);
    if (!deletedField) {
      return res.status(404).json({
        message: "Required field not found",
      });
    }
    const updateRequiredField = await RequiredField.findByIdAndUpdate(
      req.params.id,
      {
        $set: {
          status: "Inactive",
