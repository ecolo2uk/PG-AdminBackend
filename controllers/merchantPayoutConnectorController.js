...
export const getMerchantPayoutConnectorAccounts = async (req, res) => {
  try {
    const { merchantId } = req.params;
    // console.log("🔍 Fetching connector accounts for merchant:", merchantId);

    const accounts = await MerchantPayoutConnectorAccount.find({ merchantId })
...
export const addMerchantPayoutConnectorAccount = async (req, res) => {
  try {
    const { merchantId } = req.params;
    ...
    const existingAccount = await MerchantPayoutConnectorAccount.findOne({
      merchantId,
      connectorAccountId,
    });
    ...
    if (isPrimary) {
      await MerchantPayoutConnectorAccount.updateMany(
    ...
    const newAccount = new MerchantPayoutConnectorAccount({
      merchantId,
      connectorId,
      connectorAccountId,
      terminalId,
    ...
    const populatedAccount = await MerchantPayoutConnectorAccount.findById(
      savedAccount._id
    )
      .populate("connectorId", "name connectorType")
      .populate("connectorAccountId", "name currency")
...
export const updateMerchantPayoutConnectorAccount = async (req, res) => {
  try {
    ...
    const account = await MerchantPayoutConnectorAccount.findById(accountId);
    if (!account) {
      return res.status(404).json({
        success: false,
        message: "Connector account not found",
...
export const deleteMerchantPayoutConnectorAccount = async (req, res) => {
  try {
    const { accountId } = req.params;
    ...
    // await MerchantPayoutConnectorAccount.findByIdAndDelete(accountId);
    const updatedAccount =
      await MerchantPayoutConnectorAccount.findByIdAndUpdate(
        accountId,
        {
          $set: {
            status: "Inactive",
