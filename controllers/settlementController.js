...
export const getSettlementMerchants = async (req, res) => {
  try {
    ...
    const merchants = await User.find({
      ...
      .select(
        "firstname lastname email mid unsettleBalance bankDetails status createdAt"
      )
    ...
    const formattedMerchants = await Promise.all(
      merchants.map(async (merchant) => {
        try {
          ...
          const merchantDetail = await Merchant.findOne({
            userId: merchant._id,
          });
    ...
    const validMerchants = formattedMerchants
      .filter((m) => m !== null)
...
export const processSettlement = async (req, res) => {
  ...
  try {
    ...
    for (const merchantData of selectedMerchants) {
      ...
      const merchant = await User.findById(merchantData.merchantId).session(
        session
      );
      if (!merchant) {
        await session.abortTransaction();
    ...
    for (const merchantData of selectedMerchants) {
      try {
        ...
        if (
          ...
          if (merchantDetail) {
            merchantDetail.unsettledBalance = Math.max(
              0,
              (merchantDetail.unsettledBalance || 0) -
                merchantData.settlementAmount
            );
          ...
          setTimeout(async () => {
            try {
              ...
              payout.utr = `UTR${Date.now()}${Math.random()
                .toString(36)
                .substr(2, 9)
                .toUpperCase()}`;
              await payout.save();
              ...
              // console.log(
              //   `✅ Settlement completed for ${merchantData.merchantName} - UTR: ${payout.utr}`
              // );
            } catch (error) {
        ...
        console.error(
          `❌ Error processing settlement for ${merchantData.merchantName}:`,
          error
        );
        failedSettlements.push({
          merchant: merchantData.merchantName,
    ...
    // console.log(
    //   `✅ Settlement ${settlement.batchId} processed: ${payoutTransactions.length} successful, ${failedSettlements.length} failed`
    // );
...
export const getMerchantSettlementDetails = async (req, res) => {
  try {
    ...
    const merchant = await User.findById(merchantId).select(
      "firstname lastname email unsettleBalance"
    );
    ...
    // console.log(
    //   `✅ Found ${settlementDetails.length} settlements for ${merchant.firstname}`
    // );
