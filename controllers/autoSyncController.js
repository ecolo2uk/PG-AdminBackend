    ...
    for (const merchant of merchants) {
      try {
        ...
        if (syncResult.success) {
          ...
          console.log(`✅ Synced ${paymentCount} payments + ${payoutCount} payouts for ${merchant.merchantName}`);
        } else {
          results.push({
            merchantId: merchant._id,
            merchantName: merchant.merchantName,
...
export const syncMerchantTransactions = async (req, res) => {
  try {
    ...
    if (syncResult.success) {
      res.status(200).json({
        success: true,
        message: `Successfully synced ${merchant.paymentTransactions?.length || 0} payments and ${merchant.payoutTransactions?.length || 0} payouts`,
        data: {
          merchantId: merchant._id,
          merchantName: merchant.merchantName,
...
export const addTransactionToMerchant = async (merchantUserId, transactionData, type) => {
  try {
    ...
    if (result.success) {
      console.log(`✅ Auto-added ${type} transaction for merchant: ${merchant.merchantName}`);
    }
