...
const generateTerminalId = () => {
  return 'PGID' + Date.now().toString().substr(-7) + Math.floor(Math.random() * 100).toString().padStart(2, '0');
};
...
export const addMerchantConnectorAccount = async (req, res) => {
  try {
    ...
    console.log("💰 Adding connector account for merchant:", merchantId, req.body);

    // Validation
    if (!connectorId || !connectorAccountId || !industry) {
      return res.status(400).json({
        success: false,
        message: 'Connector, Connector Account, and Industry are required fields'
      });
    }
