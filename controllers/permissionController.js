  ...
  try {
    ...
    const formattedData = modules.map((module) => {
      const moduleSubmodules = submodules
        ...
        .map((sub) => {
          const currentPermission = existingPermissions.find(
            (p) => p.submoduleId.toString() === sub._id.toString()
          );
          return {
            _id: sub._id,
    ...
    console.error(
      "Error fetching modules, submodules, and permissions:",
      error
    );
    res
      .status(500)
      .json({ message: "Server error while fetching permissions." });
  }
};
  ...
  try {
    ...
    for (const p of permissions) {
      ...
      if (!mongoose.Types.ObjectId.isValid(p.submoduleId)) {
        console.warn(
          `Skipping permission for invalid submoduleId: ${p.submoduleId}`
        );
        continue;
      }

      // Filter to valid operations - your current code does this, good.
      const validOperations = p.operations.filter((op) =>
        ["read", "create", "update", "delete"].includes(op)
      );

      if (validOperations.length > 0) {
        // Only add if there are valid operations
...
export const getAllModulesAndSubmodules = async (req, res) => {
  try {
    ...
    const formattedData = modules.map((module) => {
      return {
        ...
        submodules: submodules
          .filter(
            (sub) => sub.moduleId._id.toString() === module._id.toString()
          )
          .map((sub) => ({
            _id: sub._id,
    ...
    res
      .status(500)
      .json({ message: "Server error while fetching modules and submodules." });
  }
};
