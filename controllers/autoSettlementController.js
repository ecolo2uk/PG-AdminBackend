...
export const getAutoSettlements = async (req, res) => {
  try {
    ...
    const autoSettlements = await AutoSettlement.find(query)
      .populate(
        "connectorId",
        "name className connectorType status isPayoutSupport"
      )
...
export const toggleStatus = async (req, res) => {
  try {
    ...
    res.status(200).json({
      success: true,
      message: `Auto settlement ${
        autoSettlement.isActive ? "activated" : "deactivated"
      }`,
      data: autoSettlement,
    });
  } catch (error) {
      ...
      if (merchant.unsettledBalance >= autoSettlement.minimumAmount) {
        try {
          ...
          const payoutTransaction = new PayoutTransaction({
            payoutId: `P${Date.now()}${Math.random()
              .toString(36)
          ...
          // console.log(
          //   `✅ Settled ${previousUnsettled} for merchant: ${merchant.merchantName}`
          // );
        } catch (merchantError) {
          console.error(
            `❌ Error settling merchant ${merchant.merchantName}:`,
            merchantError
          );
          failedSettlements++;
...
const scheduleAutoSettlement = (autoSettlement) => {
  try {
    ...
    // console.log(
    //   `⏰ Scheduled auto settlement: ${autoSettlement.name} at ${autoSettlement.cronRunTime}`
    // );
  } catch (error) {
